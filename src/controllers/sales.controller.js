...
const { SALE_CHANNELS, MOVEMENT_TYPES } = require("../constants");
...
async function create(req, res) {
  ...
  const sale = await prisma.$transaction(async (tx) => {
    // Verificar y descontar stock atomicamente (mismo patron que en pedidos online).
    ...
    for (const { product, quantity } of itemsWithProduct) {
      await tx.inventoryMovement.create({
        data: { productId: product.id, type: MOVEMENT_TYPES.VENTA, quantity, reason: `Venta presencial #${createdSale.id}`, userId: req.user.id, saleId: createdSale.id },
      });
    }
...
async function voidSale(req, res) {
  ...
  const sale = await prisma.$transaction(async (tx) => {
    ...
    for (const item of existing.items) {
      ...
      await tx.inventoryMovement.create({
        data: { productId: item.productId, type: MOVEMENT_TYPES.DEVOLUCION, quantity: item.quantity, reason: `Anulacion de venta #${id}`, userId: req.user.id, saleId: id },
      });
    }
...
module.exports = { create, list, voidSale };
